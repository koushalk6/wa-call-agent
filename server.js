...
async function handleOffer(callId, offerSDP, phoneNumberId) {
  ...
  // Silent keepalive + backup loop text
  const silentInterval = startSilentAudio(audioSource);

  const backupTexts = ["Hi, how are you?", "Hello there!", "Testing voice.", "Keep talking.", "AI voice loop."];
  const backupInterval = setInterval(async () => {
    const text = backupTexts[Math.floor(Math.random() * backupTexts.length)];
    await streamTTS(text, audioSource);
...
function cleanup(callId) {
  ...
  try {
    ...
    clearInterval(c.backupInterval);
  } catch (_) {}
